# -*- coding: utf-8 -*-
"""GCN_sample_code.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1wmWvB1XU0YfaR837OQvNkOIkkiEKRR36

### Sample code to generate a GCN model for Serotonin Transporter
"""
...
csv_featurized_dir = my_directory/'csv_featurized'
if not csv_featurized_dir.is_dir():
    csv_featurized_dir.mkdir()
    
train_dir = csv_featurized_dir/'train_dir'
if not train_dir.is_dir():
    train_dir.mkdir()
    
valid_dir = csv_featurized_dir/'valid_dir'
if not valid_dir.is_dir():
    valid_dir.mkdir()

test_dir = csv_featurized_dir/'test_dir'
if not test_dir.is_dir():
    test_dir.mkdir()
    
target_joblib_dir = my_directory/'target_joblib'
if not target_joblib_dir.is_dir():
    target_joblib_dir.mkdir()

model_dir = my_directory/'models'
...
# Adjust a file name
input_csv = my_directory/'SEROTONIN_TRANSPORTER_demo.csv'
dataset = loader.featurize(str(input_csv), data_dir=str(csv_featurized_dir))

# Split the dataset

splitter = RandomSplitter()

train_val_data, test_data = splitter.train_test_split(
        dataset, 
        test_dir=str(test_dir),
        frac_train=0.9, 
        verbose=False)
     
train_data, valid_data = splitter.train_test_split(
        train_val_data,
...
# Access files
dc.utils.save.save_to_disk(
        train_data,
        str(target_joblib_dir/'train_data.joblib')
        )

dc.utils.save.save_to_disk(
        valid_data,
        str(target_joblib_dir/'valid_data.joblib')
        )

dc.utils.save.save_to_disk(
        test_data,
        str(target_joblib_dir/'test_data.joblib')
        )
...
def hyper_model(dense_layer_size, graph_conv_layer_size, batch_size, dropout, nb_epoch, **params):
    ...
    # MAE
    valid_scores_mae = model.evaluate(valid_data, [mean_absolute_error], per_task_metrics=False)
    # R2
    valid_scores_r2 = model.evaluate(valid_data, [pearson_r2_score], per_task_metrics=False)
    # RMSE
    valid_scores_rmse = model.evaluate(valid_data, [rms_score], per_task_metrics=False)
    # -MAE
...
train_data = load_from_disk(str(target_joblib_dir/'train_data.joblib'))
valid_data = load_from_disk(str(target_joblib_dir/'valid_data.joblib'))
test_data  = load_from_disk(str(target_joblib_dir/'test_data.joblib'))
